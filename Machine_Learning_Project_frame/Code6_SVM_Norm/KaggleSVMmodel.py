...
class SupportVectorMachine(Model):
    ...
    def train(self, x: np.ndarray, y: np.ndarray, epochs: int):
        ...
        if self.plotting:
                ...
                plt.title("SVM: Average Loss per Epoch")
                plt.xlabel("Epoch")
                plt.ylabel("Average Loss")
                plt.grid(True)
                plt.tight_layout()
                plt.savefig(f'svm_loss_plot_features_{x.shape[1]}.png', dpi=300)
                plt.show()        
...
class LogisticRegression(Model):
    ...
    def train(self, x: np.ndarray, y: np.ndarray, epochs: int):
        ...
        if self.plotting:
                ...
                plt.grid(True)
                plt.tight_layout()
                plt.savefig(f'Logistic_loss_plot_features_{x.shape[1]}.png', dpi=300)
                plt.show()        
